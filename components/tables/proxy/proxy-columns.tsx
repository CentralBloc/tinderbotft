...
export const EditableProxyCell = ({row, view}: Readonly<UdpdateStratOrAccountPropsInterface>) => {
  ...
  const updateThreadAccountMutation = useUpdateThreadAccount(row.original.id);
  ...
  const handleProxyChange = (newProxyId: string) => {
    if (view === "strategy") {
      ...
      updateThreadAccountMutation.mutate({proxy: newProxyId});
    }
  };
